...
| 依存の有無 | `package.json` の `dependencies` を確認 | 存在しない、または空 |
...
- **テストフレームワークの変更提案。** `unittest` は依存ゼロを保つための意図的な選択。pytest を勧めない
- **依存の追加提案。** 「このライブラリを使えば短く書ける」は、このパッケージでは改悪
- **描画エンジンの別言語での書き直し。** Python であることが設計の中核
- **日本語のコメントと docstring。** コード内のコメントは日本語が規約。英語にするのは利用者に見える出力（stderr メッセージ、CLI のヘルプ、README）のみ
